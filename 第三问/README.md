...
为完整保留官方原始结果并控制仓库体积，本仓库的每个 `best_result.json` 均以无损 `best_result.json.gz` 保存。`run_problem3_all.py` 会跳过已有的压缩结果；`validate_results.py` 可直接读取并验证，无需先解压。若需要展开单个文件，可用 Python 标准库 `gzip`。候选过程中的临时计划和追踪文件未纳入交付，最终方案、官方原始结果、搜索历史和汇总统计均已保留。
